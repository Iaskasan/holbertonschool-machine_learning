...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        ----------
        image : numpy.ndarray
            The original image on which to draw the boxes.
        boxes : numpy.ndarray of shape (N, 4)
            Array of bounding boxes in the format (x1, y1, x2, y2).
        box_classes : numpy.ndarray of shape (N,)
            Array of class indices for each box.
        box_scores : numpy.ndarray of shape (N,)
            Array of confidence scores for each box.
        file_name : str
            The name of the file to display as the window title.
        """
        ...
        key = cv2.waitKey(0)
        if key == ord('s'):
            # Save the image with boxes if 's' is pressed
